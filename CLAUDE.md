...
- `base_duckdb.py` — a base da empresa (leitura dos dois parquet, join com o catálogo, corte D-1 e toda a limpeza de `validar_e_limpar`) numa consulta DuckDB, saindo pelo Arrow. O DataFrame é **idêntico** ao do caminho pandas — conferido coluna a coluna nas 41 empresas em set/2026 e travado em `tests/test_base_duckdb.py` —, e custa 1,4 s na IBAD contra 5,6 s (41 empresas: 27 s contra 77 s). Esquema fora do padrão (código que não é texto, coluna faltando, mês fora de 1–12) e erro de leitura (OneDrive travando, placeholder) caem em `carregar_pandas`, que tem as mensagens de sempre. Duas coisas que a equivalência ensinou: o `trim` do DuckDB só tira espaço, e o `str.strip` do Python tira qualquer branco (a IBAD tem cliente com tabulação no fim) — daí a macro `py_strip`; e código vazio nunca é chave de join, dos dois lados (a Mega tem no catálogo uma linha de código só com espaços, que dava "Coxim Motor" a vendas sem código). O `_cache_atacado.parquet`, que guardava o join na pasta de trabalho desde o tempo do CSV, saiu: a consulta inteira custa menos do que relê-lo e limpar em pandas.
...
A fonte é **só parquet** desde set/2026, com o mesmo esquema nas 41 empresas: identificador `string`, `DATA_MOVIMENTO`/`DATA_VENC` `date32`, `DIA`/`MES`/`ANO` `int32`, valores `double`. CSV que sobre na pasta é ignorado. O movimento da Altese caiu de 325 MB para 10,6 MB, e carregar a base foi de 14,5 s para 1,5 s (IBAD, 1,46 milhão de linhas: ~19 s → 1,0 s). O gargalo passou a ser a limpeza em pandas depois da leitura (`validar_e_limpar`, 5 s na IBAD), e ela foi para dentro da consulta DuckDB (`base_duckdb`, ver Backend).
...
**A base vai sempre até ontem.** `af.data_corte_padrao()` (D-1; `PRISMA_DATA_CORTE=AAAA-MM-DD` força outra data) e `af.cortar_ate` tiram as linhas de `Data_Venda_Diaria` depois do corte. O dia corrente chega parcial, e "hoje até agora" contra dias cheios faz toda tela ler queda. O corte roda dentro da consulta de `base_duckdb` (e em `af.cortar_ate` no caminho pandas), e a data entra na chave da base em RAM e no `summary_dashboard.corte`, que o frescor do summary confere — virou o dia, tudo que foi calculado com o corte velho deixa de valer mesmo sem a fonte mudar. O "Último movimento" também é limitado ao corte.

**Telas pesadas têm cache em disco** (`backend/cache_telas.py`, `{trabalho}/{empresa}/_cache_telas/`): Clientes, Diagnóstico, Vendedores, Estoque (resumo e mapa) e a abertura das duas abas da Precificação (A precificar ~3–4 s e Pós-precificação ~1,5 s a frio; o histórico só vai ao disco sem rodada, faixa ou busca). A chave é a mesma tupla que a tela já usava na RAM (empresa, loja, modo, assinatura da base, dia, cortes, tags), mais corte D-1 e regra de nomes; ela vira hash no nome do arquivo e fica gravada dentro, conferida na leitura. Qualquer mudança que invalidaria a RAM muda a chave, e o arquivo antigo não é mais achado. Antes, cada máquina recalculava cada tela na primeira abertura do dia (Clientes ~8 s na IBAD).

**Tudo na chave precisa ser estável entre processos**, senão o que o lote grava nunca é lido. Os arquivos de tags entram pelo **conteúdo** (`_assinatura_conteudo_opcional`), não pelo mtime: o `Bancos/tags.json` centralizado era regravado igual a cada salvamento de tags de qualquer empresa, a chave de Clientes mudava a cada poucos minutos e a IBAD abria em 7,5 s com o arquivo do lote parado ao lado (set/2026). O catálogo centralizado também deixou de ser regravado quando não muda.
...
- **GPS na aba A precificar** (`gps_dispersao.py`, tabela em `gps_logica.py`): as métricas da aba Dispersão do GPS (Power BI), sem mudança de cálculo, **só para as 36 descrições** da tabela 2D (`Logica do GPS.xlsx`, aba Lógica; `tests/test_gps_dispersao.py` confere a planilha quando ela está na máquina). Taxa de Retorno = margem − despesas dos **3 últimos meses fechados**, com despesa = Controladoria sem "Mercadoria Revenda" (Dividendos entram) → perfil. Dispersão = margem da descrição − margem geral na janela do A precificar; margem geral e total da participação usam a receita inteira do PRICE (o `ALL` do DAX). Decisões que o GPS não trazia: MAIOR = limite da faixa mais longe de zero, MENOR = mais perto; ajuste = **distância** do limite-alvo à dispersão atual (somar o limite à margem afastava metade dos itens da faixa); "Acima da Média" vai até a borda mais próxima, subindo no máximo o "até X" do cenário; teto = largura da faixa (um degrau por rodada). O ajuste cruza com as provas em 7 recomendações (Reajustar, Subir em etapas, Oportunidade, Divergência, Reduzir, Segurar preço, Manter; limites 15% de volume e 2 pp de custo sem repasse). **A lista da tela é por produto (descrição)** — o grão do GPS —, em `produtos`, e cada produto traz os `fabricantes` (descrição × fabricante, o grão do PRICE), que a tela abre embaixo da linha ao clicar; o fabricante leva o mesmo ajuste do produto, cruzado com as provas dele. Produto e fabricante que só o GPS aponta vêm com `sinalizado = False`, depois dos sinalizados; o produto só aparece com o filtro de recomendação ligado. `/a-precificar/par` sem `fabricante` devolve o produto inteiro.
